              ...
              polygon?.map((item, index) => {
                ...
                return (
                  <div className='input-item' key={'draw-input-item' + index}>
                    ...
                    <div className='flex'>
                      <p>X : </p>
                      <input type="number" step={0.1} min={0} max={22} value={Number(item.x * unitWidth * maxWidthNum / 100).toFixed(1)}
                        onChange={(e) => {
                          // console.log("from", tempPolygon)
                          // console.log("step", e.target.value*100/(unitWidth*maxWidthNum))
                          // tempPolygon[index].x = e.target.value*100/(unitWidth*maxWidthNum)
                          // console.log("to",tempPolygon)
                          // setPolygon(tempPolygon)
                        }} />
                      <p>Y : </p>
                      <input type="number" step={0.1} min={0} max={10.5} value={Number((100-item.y) * unitHeight * maxHeightNum / 100).toFixed(1)}
                        onChange={(e) => {
                          // console.log("from", tempPolygon)
